  ...
  const errorMsg = ref('')
    ...
    async function startCamera () {
      try {
        ...
        // Fallback: use html5-qrcode (already in package.json)
        useHtml5.value = true
        ...
        try {
          ...
          if (!started) {
            ...
            errorMsg.value = 'Kein Kamera-Fallback verfügbar: ' + msg
          ...
          supported.value = false
          errorMsg.value = 'Kein Kamera-Fallback verfügbar: ' + (e && e.message ? e.message : String(e))
          emit('error', new Error(errorMsg.value))
        }
      } catch (err) {
    supported.value = false
    errorMsg.value = 'Error getting userMedia, error = ' + (err && err.message ? err.message : String(err))
    emit('error', new Error(errorMsg.value))
      }
    }
...
<style scoped>
.inline-camera { display:flex; flex-direction:column; gap:0.5rem; }
.video-container { position:relative; width:100%; padding-top:56%; background:#000; border-radius:8px; overflow:hidden; }
